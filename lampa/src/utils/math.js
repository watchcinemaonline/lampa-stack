...
function calcUserRating(element)
{
    ...
    var matrix = [
        ["4K", 4, function () {
            return null != title.match(/\b(4K|2160p)\b/)
                && null == title.match(/\b(TS|Telesync|CAM|CAMRip)\b/i);
        }],
        ["1080p", 3, function () {
            return null != title.match(/\b(1080p)\b/)
                && null == title.match(/\b(TS|Telesync|CAM|CAMRip)\b/i);
        }],
        ["720p", 2, function () {
            return null != title.match(/\b(720p)\b/)
                && null == title.match(/\b(TS|Telesync|CAM|CAMRip)\b/i);
        }],
        ["DUB_RUS", 3, function () {
            if (null != title.match(/\bRUS(SIAN){0,1}$/)) {
                if (null == title.match(/^.*\bSub.*(Russian|Rus|Ru).*RUS(SIAN){0,1}$/)) {
                    return true;
                }
            } else {
                if (null != title.match(/[^a-z]x*(Russian|Rus|Ru)\b/)
                    && null == title.match(/^.*\bSub.*\b(Russian|Rus|Ru)\b/)) {
                    return true;
                }
            }
        ...
        ["DUB-RUS-SUB-RUS", 3, function () {
            if (null != title.match(/\bRUS(SIAN){0,1}$/)) {
                if (null != title.match(/^.*\bSub.*(Russian|Rus|Ru).*RUS(SIAN){0,1}$/)) {
                    return true;
                }
            } else {
                if (null != title.match(/(\b(Dub)*\b).*?[^a-z]x*(Russian|Rus|Ru)\b/)
                    && null != title.match(/^.*\bSub.*\b(Russian|Rus|Ru)\b/)) {
                    return true;
                }
            }
        ...
        ["DUB-UKR", 3, function () {
            return null != title.match(/[^a-z]x*(Ukrainian|Ukr|Uk)\b/) 
                && null == title.match(/(\bSub\b).*(\b(Ukr|Uk)\b)/) 
        }],
        ["DUB-UKR-SUB-UKR", 3, function () {
            return null != title.match(/(\b(Dub)*\b).*?[^a-z]x*(Ukrainian|Ukr|Uk)\b.*?(\b(Sub)\b).*?[^a-z]x*(Ukrainian|Ukr|Uk)\b/) 
        }],
        ["Director's Cut", 1, function () {
            return null != title.match(/(Director\'*s*.*?Cut|extended)/) 
        }],
        ["ENG", 1, function () {
            return null != title.match(/[^a-z]x*(Eng|En)\b/) 
                && null == title.match(/(\bSub\b).*(\b(Eng|En)\b)/) 
        }],
        ["ENG-SUB-ENG", 1, function () {
            return null != title.match(/[^a-z]x*(Eng|En)\b/) 
                && null != title.match(/(\bSub\b).*(\b(Eng|En)\b)/) 
        }],
        ["HDR", 1, function () {
            return null != title.match(/\b((HDR[^\s\w]*[0-9]*))\b/);
        }],
        ["HEVC", 1, function () {
            return null != title.match(/\b((HEVC)|([hx][^\s\w]*265)|(HDR[^\s\w]*[0-9]*)|(Dolby\s*Vision))\b/)
        }],
    ]

    for(let i = 0; i < matrix.length; i++){
        try {
            if ('undefined' != typeof matrix[i] 
                && 'undefined' != typeof matrix[i][0] 
                && 'undefined' != typeof matrix[i][1] 
                && 'undefined' != typeof matrix[i][2] 
                && matrix[i][2]()) {
                userRatingValue += parseInt(matrix[i][1]);
                userRatingTags.push(matrix[i][0]);
            }
        } catch (e) {
            console.log('User Rating Calc Error (' + matrix[i][0] + '): ' + e);
        }
    }

    return {userRating: userRatingValue, userRatingTags: userRatingTags};
}
