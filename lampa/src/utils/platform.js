...
function init(){
    ...
    else if(agent.indexOf("linux") > -1 || agent.indexOf("windows nt") > -1 || (agent.indexOf("macintosh") > -1 && !Utils.isTouchDevice())) {
 ...
 */
function any(){
    return is('tizen') || is('webos') || is('android') || is('netcast') || is('orsay') || is('apple') || is('apple_tv') || macOS() || windows() || linux() || desktop() ? true : false
}
...
function windows(){
    ...
    return agent.indexOf("phone") == -1 && agent.indexOf("mobi") == -1 && agent.indexOf("windows") > -1 && !Utils.isTouchDevice()
}

function linux(){
    const agent = navigator.userAgent.toLowerCase()

    return agent.indexOf("phone") == -1 && agent.indexOf("mobi") == -1 && agent.indexOf("linux") > -1 && !Utils.isTouchDevice()
}
