...
function component(object){
    ...
    let sort_translate = {
        UserRating: Lang.translate('torrent_parser_sort_by_user_rating'),
    ...
    this.buildUserRating = function () {
        results.Results.forEach(element => {
            const userRating = Utils.calcUserRating(element);
            Arrays.extend(element,{
                UserRating: userRating.userRating,
                UserRatingTags: userRating.userRatingTags.join(',')
            });
        });
    }

    this.buildSorted = function(){
        let need   = Storage.get('torrents_sort','UserRating')
        let select = [
            {
                title: Lang.translate('torrent_parser_sort_by_user_rating'),
                sort: 'UserRating'
            },
            {
    ...
    this.sortWithUserRating = function(){
    ...
    this.selectedSort = function(){
        let select = Storage.get('torrents_sort','UserRating')

        filter.chosen('sort', [sort_translate[select]])
    }

    this.build = function(){
        this.buildUserRating();
        this.buildSorted()
        this.buildFilterd()
